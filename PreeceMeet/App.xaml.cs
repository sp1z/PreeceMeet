...
public partial class App : Application
{
    ...
    private async Task<bool> RunLoginFlowAsync()
    {
        ...
        _session.Save(new SavedSession
        {
            ...
            // The server should also return a long-lived session token alongside
            // the short-lived LiveKit JWT.  We store it under SessionToken.
            // If the API only returns a LiveKit JWT, store it as a short-lived
            // session and the user will be prompted again after it expires.
            SessionToken  = result.LiveKitToken,
